...
| 🧯 Failure Analysis | Nine build failures documented with symptom, cause, fix and lesson |
...
**Domain filtering works, and the reason it took three attempts is the useful part.** Both listed
domains now resolve to the sinkhole address 10.10.10.1 while an unrelated control domain answers
normally in the same session. Getting there meant clearing two failures that are both completely
silent. The first was the reload — settings apply on a reload run, not on save, so until one had
completed the group did nothing at all. The second was the **DNSBL whitelist**, which sits further
down the same settings page and held `youtube.com` and `www.youtube.com` but not `instagram.com`.

> ⚠️ **A whitelist entry overrides a block group with no error and no indication anywhere near the
> group it contradicts.** That single fact explains why one domain sinkholed and an identically
> configured one did not — and a split result like that is a pointer at the exception list, never at
> the mechanism.

There is a deeper limit that applies even when this works perfectly: a browser can resolve names over
its own encrypted connection to a public resolver, never asking the firewall at all. The package can
answer *no such domain* for the published DNS-over-HTTPS endpoints, which narrows the gap, and it was
left disabled here. **Domain filtering at a perimeter is a control that works on cooperative clients
...
| 12 | Domain filtering sinkholes listed domains | Name lookups from the endpoint, with a control domain | ✅ Both resolve to 10.10.10.1, after a whitelist entry was found and removed |

**The two results that are not a tick mark are the most useful in the set**, and both are explained
in full in the report rather than being dropped from it. So is the state where domain filtering
blocked one name and not the other, because a build described only by its successes is not a
...
| 8 | One listed domain sinkholes and an identically configured one does not | The second sat in the DNSBL whitelist, which overrides a block group silently | Removed the entries and ran a forced reload |
| 9 | Validation passes, alerts are written, dashboard is empty | A filter left pinned on the search, and two machines five hours apart in their time zones | Cleared the filter, set both machines to the same time zone |
...
A MISSING FIELD CAN DEFEAT A WORKING DECODER
        ...
        ├── A split result points at the exception list, not at the mechanism
...
* **Switch on the encrypted resolver blocking**, which answers *no such domain* for the published
  DNS-over-HTTPS endpoints and narrows the gap left by domain filtering
...
│
├── docs/
│   └── Perimeter-Defence-Home-Lab.pdf       # full 46-page report, 38 labelled figures
│
├── detections/
...
The complete report in **`docs/`** runs to 46 pages with 38 labelled figures across four parts:
