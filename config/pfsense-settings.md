...
**Domain filtering did not work in this build**, and is reported as unresolved rather than as
working. A virtual address of `10.10.10.1/32` was created on the loopback interface for the sinkhole
web server and a block group created with the resolver-based action, but name lookups from the
endpoint continued to return the real public addresses for both listed domains. Two known causes
produce exactly that symptom and neither could be confirmed or excluded:

* The sinkhole address not present on the loopback interface, in which case the package disables
  domain blocking and records the reason in its own update log rather than anywhere in the
  interface, so the configuration continues to look correct while doing nothing.
* The safe-search option, which works by resolving a domain to a filtered address. It guarantees the
  name is answered, which is the opposite of sinkholing it.

There is also a limit that applies even when this is configured perfectly: a browser can resolve
